...
describe('createPost', () => {
   ...
   it('succeeds when the post is created', async () => {
      ...
      await createPost(
         user.id,
         'Test post',
         'https://imgs.search.brave.com/wWVzfAneB1zJ8mc1yhf5F4YTISoTGd_2aPl-MG_wJSU/rs:fit:860:0:0:0/g:ce/aHR0cHM6Ly9pbWFn/ZXMuZnJlZWltYWdl/cy5jb20vaW1hZ2Vz/L2xhcmdlLXByZXZp/ZXdzLzllMS90ZXN0/LTQtMTQ4NjQ2MC5q/cGc_Zm10',
         'Post description'
      );
   ...
   it('fails when the user is not found', async () => {
      await expect(createPost('66998f609e4a02acb2927f4e', 'title', 'https', 'description')).to.be.rejectedWith(
         NotFoundError,
         'user not found'
      );
   });
