...
describe('deletePost', () => {
   ...
   it('succeeds when the post is deleted', async () => {
      ...
      const post = await Post.create({
         author: user.id,
         title: 'Post title',
         image: 'https://imgs.search.brave.com/wWVzfAneB1zJ8mc1yhf5F4YTISoTGd_2aPl-MG_wJSU/rs:fit:860:0:0:0/g:ce/aHR0cHM6Ly9pbWFn/ZXMuZnJlZWltYWdl/cy5jb20vaW1hZ2Vz/L2xhcmdlLXByZXZp/ZXdzLzllMS90ZXN0/LTQtMTQ4NjQ2MC5q/cGc_Zm10',
   ...
   it('fails when the user is not found', async () => {
      await expect(deletePost('66998f609e4a02acb2927f4e', '669c2f04a028848eb0aeb742')).to.be.rejectedWith(
         NotFoundError,
         'user not found'
      );
   });
   ...
   it('fails when the post is not found', async () => {
      ...
      await expect(deletePost(user.id, '669c2f04a028848eb0aeb742')).to.be.rejectedWith(NotFoundError, 'post not found');
   });
   ...
   it('fails when the post author does not match the user', async () => {
      ...
      await expect(deletePost(user2.id, post.id)).to.be.rejectedWith(MatchError, 'post author does not match user');
   });
