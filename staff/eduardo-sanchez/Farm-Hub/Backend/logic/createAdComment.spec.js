...
describe('createAdComment', () => {
                    ...
                    .then(ad =>
                        createAdComment(user.id, ad.id, 'Nice Ad!')
                            ...
                            .then(ad => {
                                ...
                                expect(ad.adcomments[0].comment).to.equal('Nice Ad!')
    ...
    it('fails on non-existing user', () => {
        ...
        return Ad.create({ author: new ObjectId(), title: 'Limones', description: 'Luneros', price: '8.5 €/Kg', date: new Date(), adcomments: [] })
            .then(ad =>
    ...
    it('fails on invalid userId', () => {
        ...
        try {
            createAdComment('invalid-id', new ObjectId().toString(), 'Nice Ad!')
        } catch (error) {
            errorThrown = error
        } finally {
    ...
    it('fails on invalid adId', () => {
        ...
        try {
            createAdComment(new ObjectId().toString(), 'invalid-id', 'Nice Ad!')
        } catch (error) {
            errorThrown = error
        } finally {
            expect(errorThrown).to.be.instanceOf(ContentError)
    ...
    it('fails on invalid comment text', () => {
        ...
        try {
            createAdComment(new ObjectId().toString(), new ObjectId().toString(), '')
        } catch (error) {
            errorThrown = error
        } finally {
            expect(errorThrown).to.be.instanceOf(ContentError)
