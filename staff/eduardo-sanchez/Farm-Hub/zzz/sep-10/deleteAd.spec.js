...
describe('deleteAd', () => {
    ...
    it('succeeds on deleting an ad by an existing user', () =>
    ...
    it('fails on non-existing user', () => {
        ...
        return Ad.create({ author: new ObjectId(), title: 'Limones', description: 'Luneros', price: '8.5 €/Kg', date: new Date() })
            .then(ad =>
    ...
    it('fails on ad author does not match user wanting to delete ad', () => {
        ...
        return bcrypt.hash('123123123', 8)
            .then(hash => {
                ...
                const ad = new Ad({
                    author: new ObjectId().toString(), // Ensure this is different from user._id
                    title: 'Limones',
                    description: 'Luneros',
                    price: '10 €/Kg',
                    date: new Date(),
                ...
                return Promise.all([user.save(), ad.save()])
                    .then(([savedUser, savedAd]) => {
                        return deleteAd(savedUser.id.toString(), savedAd.id.toString())
                            .catch(error => errorThrown = error)
                    })
            })
            .finally(() => {
