...
describe('modifyTaskAsOwner', () => {
    ...
    it('succeeds on modify task as owner', () =>
        bcrypt.hash('123123123', 8)
            ...
            .then(({ user, task }) => 
                modifyTaskAsOwner(user.id, task.id, 'inProgress', 'observando')
            )
            .then(() => Task.findOne())
            .then(task => {
    ...
    it('fails on non-existing user', () => {
        ...
        return modifyTaskAsOwner(new ObjectId().toString(), new ObjectId().toString(), 'inProgress', 'observando')
    ...
    it('fails on non-existing task', () => {
        ...
        return bcrypt.hash('123123123', 8)
          ...
          .then((user) => modifyTaskAsOwner(user.id, user.id, 'inProgress', 'observando')))
          .catch(error => errorThrown = error)
          .finally(() => {
    ...
    it('fails on invalid user id', () => {
        ...
        try {
            modifyTaskAsOwner(1234, new ObjectId().toString(), 'inProgress', 'observando')
        } catch (error) {
            errorThrown = error
        } finally {
    ...
    it('fails on invalid task id', () => {
        ...
        try {
            modifyTaskAsOwner(new ObjectId().toString(), 1234, 'inProgress', 'observando')
        } catch (error) {
            errorThrown = error
        } finally {
            expect(errorThrown).to.be.instanceOf(ContentError)
    ...
    it('fails on invalid status', () => {
        ...
        try {
            modifyTaskAsOwner(new ObjectId().toString(), new ObjectId().toString(), 'loquemedalagana', 'observando')
        } catch (error) {
            errorThrown = error
        } finally {
            expect(errorThrown).to.be.instanceOf(ContentError)
    ...
    it('fails on invalid observations', () => {
        ...
        try {
            modifyTaskAsOwner(new ObjectId().toString(), new ObjectId().toString(), 'inProgress', 1234)
        } catch (error) {
            errorThrown = error
        } finally {
            expect(errorThrown).to.be.instanceOf(ContentError)
