Array.prototype.splice = function (start, deleteCount) {
  if (deleteCount === 0) {
    ...
    for (var i = start, j = 2; i < start + displacement; i++, j++) {
      this[i] = arguments[j]
    }
    return []
  } else {
    ...
    var displacement = arguments.length - 2 - deleteCount
    for (var j = this.length - 1; j >= start + deleteCount; j--) {
      this[j + displacement] = this[j]
    }
    for (var i = start; i < start + arguments.length - 2; i++) {
      this[i] = arguments[2 + i - start]
    }
    return removed
  }
}
