...
const getPlaylistInfo = playlistId => {
   validate.inputs(playlistId);
   validate.objectId(playlistId);

   return (async () => {
      let userToken, res, body, playlistInfo;
      ...
      try {
         res = await fetch(`${Config.API_URL}/api/v1/playlist/${playlistId}`, {
      ...
      if (res.status === 200) {
         try {
            ...
            throw new ParseError(`Playlist info parse failed: ${error.message}`);
         }
