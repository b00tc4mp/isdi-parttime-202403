...
///////////Sin destructurar jwt or SystemError (directamente usa jwt.JsonWebTokenError y jwt.TokenExpiredError). En lugar de utilizar SystemError.name, se utiliza 'SystemError' directamente en la respuesta JSON.
...
api.post('/users/auth', jsonBodyParser, (req, res) => {
    ...
    try {
        logic.authenticateUser(username, password, (error) => {
            ...
            jwt.sign({ sub: username }, JWT_SECRET, { expiresIn: '1h' }, (error, token) => {
                if (error) {
                    res.status(500).json({ error: 'JsonWebTokenError', message: error.message })
                    return
                }
