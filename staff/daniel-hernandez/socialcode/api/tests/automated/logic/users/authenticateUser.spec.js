...
describe("authenticateUser", () => {
  ...
  it("succeeds when the user already exists", async () => {
    ...
    await expect(authenticateUser("eva00", "123123123"))
  ...
  it("fails when the user does not exist", async () => {
    await expect(
      authenticateUser("nonexistent", "123123123"),
    ).to.be.rejectedWith(CredentialError, "user not found");
  });
  ...
  it("fails when the password is incorrect", async () => {
    ...
    await expect(authenticateUser("eva00", "wrongpassword")).to.be.rejectedWith(
      CredentialError,
      "wrong password",
    );
  });
  ...
  it("fails when the username is invalid", () => {
    expect(() => authenticateUser(123123123, "123123123")).to.throw(
      ContentError,
  ...
  it("fails when the password is invalid", () => {
    expect(() => authenticateUser("someone", "1231231")).to.throw(
      ContentError,
