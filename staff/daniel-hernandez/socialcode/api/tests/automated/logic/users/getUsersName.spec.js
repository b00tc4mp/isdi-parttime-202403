...
describe('getUsersName', () => {
   ...
   it("succeeds when the target user's name is returned", async () => {
      ...
      await expect(getUsersName(user1.id, user2.id)).to.eventually.be.a('string').and.equal('Asuka');
   });
   ...
   it('fails when the user is not found', async () => {
      ...
      await expect(getUsersName('66998f609e4a02acb2927f4e', targetUser.id)).to.be.rejectedWith(
         NotFoundError,
         'user not found'
      );
   });
   ...
   it('fails when the target user is not found', async () => {
      ...
      await expect(getUsersName(user.id, '66998f609e4a02acb2927f4e')).to.be.rejectedWith(
         NotFoundError,
