...
describe('likePost', () => {
   ...
   it('fails when the user is not found', async () => {
      await expect(likePost('66998f609e4a02acb2927f4e', '669c2f04a028848eb0aeb742')).to.be.rejectedWith(
         NotFoundError,
         'user not found'
      );
   });
   ...
   it('fails when the post is not found', async () => {
      ...
      await expect(likePost(user.id, '669c2f04a028848eb0aeb742')).to.be.rejectedWith(NotFoundError, 'post not found');
   });

   it('fails when the user id is not valid', () => {
      expect(() => likePost('invalid user id', '669c2f04a028848eb0aeb742')).to.throw(
         ContentError,
