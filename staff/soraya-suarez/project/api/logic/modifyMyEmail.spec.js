...
describe('modifyMyEmail', () => {
    ...
    bcrypt.hash('123123123', 8)
        ...
        .then(user =>
            modifyMyEmail(user.id, 'soraya@srz.com')
        ))
        .then(() => User.find())
        .then(users => {
    ...
    it('fails on non-existing user', () => {
        ...
        return modifyMyEmail(new ObjectId().toString(), 'soraya@srz.com')
    ...
    it('fails on invalid user id', () => {
        ...
        try {
            modifyMyEmail(1234, 'soraya@srz.com')
        } catch (error) {
            errorThrown = error
        } finally {
    ...
    it('fails on invalid email', () => {
        ...
        try {
            modifyMyEmail(new ObjectId().toString(), 'soraya')
        } catch (error) {
            errorThrown = error
        } finally {
            expect(errorThrown).to.be.instanceOf(ContentError)
