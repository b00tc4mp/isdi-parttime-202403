...
export function UpdateAdForm() {
    ...
    const [ad, setAd] = useState({ _id: '', title: '', description: '', price: '' })
    ...
    const handleUpdateAd = event => {
        ...
        try {
            ...
            logic.updateAd(adId, title, description, price)
                .then(() => {
                    navigate("/")
    ...
    const chooseAdtoUpdate = () => {
        logic.getAd(adId)
            .then(({ adId, title, description, price }) => {
                // console.log(ad)
    ...
    const handleAdUpdate = event => setAd({ ...ad, [event.target.id]: event.target.value })
