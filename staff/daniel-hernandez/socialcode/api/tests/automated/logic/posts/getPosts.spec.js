...
describe('getPosts', () => {
   ...
   it('fails when the user is not found', async () => {
      await expect(getPosts('66998f609e4a02acb2927f4e')).to.be.rejectedWith(NotFoundError, 'user not found');
   });

   it('fails when the id is not valid', () => {
      expect(() => getPosts('invalid id')).to.throw(ContentError, 'Id is not valid');
   });
