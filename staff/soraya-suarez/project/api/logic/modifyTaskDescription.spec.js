...
describe('modifyTaskDescription', () => {
    ...
    it('succeeds on modify task description', () =>
        bcrypt.hash('123123123', 8)
            ...
            .then(({ user, task }) => 
                modifyTaskDescription(user.id, task.id, 'Describiendo')
            )
            .then(() => Task.findOne())
            .then(task => {
    ...
    it('fails on non-existing user', () => {
        ...
        return modifyTaskDescription(new ObjectId().toString(), new ObjectId().toString(), 'Describiendo')
    ...
    it('fails on non-existing task', () => {
        ...
        return bcrypt.hash('123123123', 8)
          ...
          .then((user) => modifyTaskDescription(user.id, new ObjectId().toString(), 'Describiendo')))
          .catch(error => errorThrown = error)
          .finally(() => {
    ...
    it('fails on invalid user id', () => {
        ...
        try {
            modifyTaskDescription(1234, new ObjectId().toString(), 'Describiendo')
        } catch (error) {
            errorThrown = error
        } finally {
    ...
    it('fails on invalid task id', () => {
        ...
        try {
            modifyTaskDescription(new ObjectId().toString(), 1234, 'Describiendo')
        } catch (error) {
            errorThrown = error
        } finally {
            expect(errorThrown).to.be.instanceOf(ContentError)
    ...
    it('fails on invalid description', () => {
        ...
        try {
            modifyTaskDescription(new ObjectId().toString(), new ObjectId().toString(), 1234)
        } catch (error) {
            errorThrown = error
        } finally {
            expect(errorThrown).to.be.instanceOf(ContentError)
